...
The project supports two Python versions:

### Option A: Python 3.14+ (Free-Threading Mode)

Python 3.14 introduces free-threading (no GIL) support, enabling true
parallelism.
...
### Option B: Python 3.13.4+ (GPU Acceleration Mode)

Python 3.13.4 supports GPU-accelerated Monte Carlo simulations with CuPy/CUDA.
...
**IMPORTANT**: You must install the TA-Lib C library before installing the
Python wrapper.

### Windows

1. Download TA-Lib from:
...
**For Python 3.14 (Free-Threading Mode):**
...
**For Python 3.13.4 (GPU Acceleration Mode):**

```bash
# Install dependencies with GPU support
uv sync --python 3.13.4 --extra gpu --prerelease=allow
...
```bash
# Check Python version (should be 3.13.4+ or 3.14+)
python --version
...
Make sure Python 3.13.4+ or 3.14+ is in your PATH:
...
# Or use uv to specify Python version:
uv sync --python 3.14
# Or
uv sync --python 3.13.4
```
...
After installation, see [QUICKSTART.md](QUICKSTART.md) to run your first
backtest!
